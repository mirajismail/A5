...
int main (int argc, char *argv[])
{
  ...
  double recvbuf[size];
  MPI_Request send_req, recv_reqs[size];
  ...
  for (i = 0; i < ROUNDS; i++) {
    if (rank == MASTER){
        ...
        for (int p = 1; p < size; p++) {
            MPI_Irecv(&recvbuf[p], 1, MPI_DOUBLE, p, 0, MPI_COMM_WORLD, &recv_reqs[p]);
        }
        ...
        // wait for workers
        MPI_Waitall(size - 1, &recv_reqs[1], &statuses[1]);

        pisum = 0;
        for (int p = 0; p < size; p++) pisum += recvbuf[p];
        ...
        // non-blocking send
        MPI_Isend(&pi, 1, MPI_DOUBLE, MASTER, 0, MPI_COMM_WORLD, &send_req);
